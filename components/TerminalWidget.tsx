...
export const TerminalWidget: React.FC = () => {
  const [history, setHistory] = useState<Array<{ cmd: string; output: string; status: string }>>([
    { cmd: '', output: 'Telecom-as-Code Operator Console [Version 1.0.4]\nType "help" to view commands.', status: 'system' }
  ]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setHistory(prev => [...prev, { cmd: input, output: res.output, status: res.status }]);
    setInput('');
  };
  ...
  const renderOutput = (output: string) => {
      ...
      if (output.trim().startsWith('{') || output.trim().startsWith('[')) {
          try {
              // We don't actually parse it, we just apply a class, but we could valid.
              // For now, simple monospace block with slightly different color for keys vs values could be done with syntax highlighting lib,
              // but let's just stick to a clean code block look.
    ...
    <div className="bg-gray-950 rounded-lg border border-gray-800 font-mono text-sm h-[450px] flex flex-col shadow-2xl overflow-hidden">
      ...
      <form onSubmit={handleSubmit} className="p-3 bg-gray-900 border-t border-gray-800 flex items-center gap-2">
        ...
        <input 
          ...
          className="flex-1 bg-transparent border-none outline-none text-gray-100 placeholder-gray-600 font-mono"
          placeholder="telecom call +1415... (or /merge, /sms)"
          autoFocus
          spellCheck={false}
        />
      </form>
