...
export class Output {
  ...
  info(msg: string) {
    if (this.isHuman) console.log(chalk.blue('INFO:'), msg);
    // Silent in JSON mode usually, or log to stderr? Keeping silent for clean JSON stdout
  }

  warn(msg: string) {
    if (this.isHuman) console.log(chalk.yellow('WARN:'), msg);
  }

  success(msg: string) {
    if (this.isHuman) console.log(chalk.green('✔'), msg);
  }
}