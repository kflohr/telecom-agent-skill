...
export const CallsAndConf: React.FC = () => {
  ...
  const refresh = () => {
    setCalls(getCalls().filter(c => c.state !== CallState.COMPLETED && c.state !== CallState.FAILED));
    setConferences(getConferences().filter(c => c.state === ConferenceState.IN_PROGRESS));
  };
  ...
  const handleHangup = (sid: string) => {
    executeHangup(sid);
    refresh();
  };

  const handleHold = (sid: string, currentHoldState: boolean) => {
    executeHold(sid, !currentHoldState);
    refresh();
  };
