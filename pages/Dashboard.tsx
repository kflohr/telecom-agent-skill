  ...
  useEffect(() => {
    const refresh = () => {
        ...
        const calls = getCalls().filter(c => 
            ...
            // Filter out calls that are currently in a conference for the "Active Legs" view to reduce noise? 
            // The spec says "Merge: puts both legs into a conference".
            // For dashboard, let's show all legs in one list or split them.
            // Let's split: "Solo Calls" vs "Conferences".
            !getConferences().some(conf => conf.participants.some(p => p.callSid === c.callSid))
        );
        setActiveCalls(calls);
        setActiveConferences(getConferences().filter(c => c.state === ConferenceState.IN_PROGRESS));
    };
